...
ProfileTimer::~ProfileTimer() {
    if (PerformanceProfiler::getInstance().is_enabled()) {
        ...
        measurement.function_name = function_name_;
        measurement.start_time = start_time_;
        measurement.end_time = end_time;
        measurement.duration = duration;
        measurement.memory_used = PerformanceProfiler::getInstance().get_current_memory_usage();
        measurement.thread_id = PerformanceProfiler::getInstance().get_current_thread_id();
...
std::vector<PerformanceMeasurement> PerformanceProfiler::get_recent_measurements(size_t count) const {
    ...
    return std::vector<PerformanceMeasurement>(
...
std::vector<std::string> PerformanceProfiler::get_slowest_functions(size_t count) const {
    ...
    for (const auto& pair : stats_) {
        function_times.push_back({pair.first, pair.second.avg_time});
    }
...
std::vector<std::string> PerformanceProfiler::get_most_called_functions(size_t count) const {
    ...
    for (const auto& pair : stats_) {
        function_calls.push_back({pair.first, pair.second.call_count});
    }
...
double PerformanceProfiler::get_total_execution_time() const {
    ...
    return total.count() / 1000.0; // Return in milliseconds
}
...
void PerformanceProfiler::print_performance_report() const {
    ...
    std::cout << "Total execution time: " << get_total_execution_time() << " ms" << std::endl;
    std::cout << "\nTop 5 slowest functions:" << std::endl;
...
void PerformanceProfiler::print_function_summary(const std::string& function_name) const {
    ...
    if (stat.call_count == 0) {
        std::cout << "No data for function: " << function_name << std::endl;
        return;
    }
    
    std::cout << "\n📊 Function: " << function_name << std::endl;
    std::cout << "Calls: " << stat.call_count << std::endl;
    std::cout << "Total time: " << stat.total_time.count() << " μs" << std::endl;
    std::cout << "Average time: " << stat.avg_time.count() << " μs" << std::endl;
    std::cout << "Min time: " << stat.min_time.count() << " μs" << std::endl;
    std::cout << "Max time: " << stat.max_time.count() << " μs" << std::endl;
    std::cout << "Average memory: " << stat.avg_memory << " bytes" << std::endl;
}
